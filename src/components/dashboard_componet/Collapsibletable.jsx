  ...
  return (
    <>
      ...
      <TableRow
        sx={{
          "& > *": { borderBottom: "unset" },
          "&:hover": { backgroundColor: "#f9fafb", transition: "0.2s" },
        }}
      >
        <TableCell>
          <IconButton
...
// ✅ Accept searchQuery and refreshKey as props
export default function CollapsibleTable({ searchQuery = "", refreshKey = 0 }) {
  const [page, setPage] = useState(1);
  const limit = 2;

  // ✅ Fetch ALL merchants for search to work across all pages
  const { data: allData } = useMerchants(1, 1000); // large limit = all merchants
  const { data, isFetching } = useMerchants(page, limit); // normal paginated fetch

  const allMerchants = allData?.data?.data || [];

  // ✅ Filter ALL merchants by search
  const filteredMerchants = searchQuery.trim()
    ? allMerchants.filter((m) => {
        const q = searchQuery.toLowerCase();
        return (
          m.name?.toLowerCase().includes(q) ||
          String(m.merchant_id)?.toLowerCase().includes(q) ||
  ...
  const merchants = filteredMerchants.slice((page - 1) * limit, page * limit);
...
//   const allMerchants = data?.data?.data || [];
//   const total = data?.data?.count || 0;
//   const totalPages = Math.ceil(total / limit);
//   // const emptyRows = limit - merchants.length;

//   // ✅ Filter merchants by search query
//   const merchants = searchQuery.trim()
//     ? allMerchants.filter((m) => {
//         const q = searchQuery.toLowerCase();
//         return (
//           m.name?.toLowerCase().includes(q) ||
